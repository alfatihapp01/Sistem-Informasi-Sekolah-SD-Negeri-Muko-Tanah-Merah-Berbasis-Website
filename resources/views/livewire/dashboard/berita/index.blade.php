        ...
        <section class="content">
            <div class="container-fluid">
                ...
                @if ($showLivewireCreate)
                    @livewire('dashboard.berita.create')
                    <script>
                ...
                @if ($showLivewireUpdate)
                    @livewire('dashboard.berita.update', ['berita' => $getBerita])
                    <script>
                        $(document).ready(function() {
                ...
                @if ($showLivewireDelete)
                    @livewire('dashboard.berita.delete', ['berita' => $getBerita])
                    <script>
                        $(document).ready(function() {
                                        ...
                                        @foreach ($berita as $data)
                                            <tr>
                                                ...
                                                <td style="vertical-align: middle;text-align: center;">
                                                    ...
                                                    wire:click="edit({{ $data->id_berita }})" ata-bs-toggle="tooltip"
                                                    data-bs-placement="bottom" title="Ubah">
                                                    <span wire:loading.remove
                                                        wire:target="edit({{ $data->id_berita }})"><i
                                                            class="bi bi-pencil-square"></i></span>
                                                    <span wire:loading wire:target="edit({{ $data->id_berita }})"
                                                ...
                                                <button style="width: 40px;" class="badge bg-danger mb-1 border-0 py-2"
                                                    wire:click="delete({{ $data->id_berita }})" ata-bs-toggle="tooltip"
                                                    data-bs-placement="bottom" title="Hapus">

                                                    <span wire:loading.remove
                                                        wire:target="delete({{ $data->id_berita }})"><i
                                                            class="bi bi-trash"></i></span>
                                                    <span wire:loading wire:target="delete({{ $data->id_berita }})"
                                                        class="spinner-border spinner-border-sm text-light"
                                                        role="status" aria-hidden="true"
                                                        style="width: 11px; height: 11px;"></span>
                                                </button>
